...
for url in "${BLACKLISTS[@]}"; do
  ...
  IP_TMP_FILE=$(mktemp -t "nft-blacklist-source-$BL_NAME-XXX")
...
if (( $OPTIMIZE_CIDR )); then
  ...
  if [[ $CIDR_MERGER =~ merger ]]; then
      ...
      $CIDR_MERGER "$IP_BLACKLIST_FILE" > "$IP_BLACKLIST_TMP_FILE"
      $CIDR_MERGER "$IP6_BLACKLIST_FILE" > "$IP6_BLACKLIST_TMP_FILE"
  fi
